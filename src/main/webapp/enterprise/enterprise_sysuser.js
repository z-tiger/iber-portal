$(function() {
	...
	function search() {
		var LoginUAcc = $("#LoginUAcc").textbox('getText');
		var LoginUName = $("#LoginUName").textbox('getText');
        var enterpriseName = $('#enterpriseName').textbox('getText');
	...
	$("#btnClear").bind("click", function() {
	 ...
	 // 添加
		...
		//角色树
		$("#roletree").tree({
			url : "/enterprise_sys_role_buildRoleTree",
			cascadeCheck : true,
			checkbox : true,
			lines : true
		});
		...
		//菜单树
		$("#cdtree").tree({
			url : "/sys_enterprise_modu_showList",
			cascadeCheck : true,
			checkbox : true,
			lines : true
		});
		...
		$("#btnEdit").bind("click",function(){
			   ...
			   if(selectedRow.length <= 0){
					$.messager.alert("提示", "请选择一条记录", "error");
					   ...
					   $('#enterprise_id').combobox('setValue',userJsonData.enterpriseId);
				...
				handler : function() {
					$("#modifyPwdForm").form("submit", {
						...
						onSubmit : function() {
						   ...
						   /* if(reg) {
                                var _newPwd = $("#newPwd").textbox("getText");
                                var _newPwd2 = $("#newPwd2").textbox("getText");
                                if(_newPwd!=_newPwd2){
                                    $.messager.alert("提示", "两次输入的新密码不一致，请重新输入", "error");
                                    reg=false;
                                }
