$(function() {
			...
			$("#btnQuery").bind("click",function(){
			    $("#dataGrid").datagrid("load",{
					'maintenancerName':$("#maintenancerName").textbox("getValue"),
			...
			$("#btnExport").bind("click", function() {
				var maintenancerName = $("#maintenancerName").textbox("getValue");
			    var cityCode = $("#cityCode").combobox("getValue");
			    var status = $("#status").combobox("getValue");
				$("#maintenancerForm").form("submit", {
					url : "export_maintenancer_report?maintenancerName="+maintenancerName+"&cityCode="+cityCode+"&status="+status,
					onSubmit : function() {
					},
		...
		buttons : [ {
			...
			handler : function() {
				$("#addViewForm").form("submit", {
					url : "addOrUpdateMaintenancerInfo",
					onSubmit : function() {
						$.messager.progress({ 
		                    text:"正在加载，请稍等！"
		                });
	...
	$("#btnAdd").bind("click",function(){
		...
		$("#e-oType").val('ADD_MAINTENANCER');
		$("#addView").dialog({title:"添加维保员",width: 700,height: 400});
		$("#addView").dialog("open");
	});
	...
	$("#btnEdit").bind("click",function(){
		...
		if(selectedRows.length <= 0){
			...
			$("#edit-id").val(selectedRows[0].id);
			$("#edit-maintenancerName").textbox('setValue',selectedRows[0].name);
			$("#edit-phone").textbox('setValue',selectedRows[0].phone);
			$("#edit-identy").combobox('setValue',selectedRows[0].identifyLabel);
			$("#edit-sex").combobox('setValue',selectedRows[0].sex);
			$("#edit-city").combobox('setValue',selectedRows[0].cityCode); 
			$("#edit-worker").combobox('setValue',selectedRows[0].profession); 
			$("#edit-type").combobox('setValue',selectedRows[0].type); 
			$("#edit-specialComment").textbox('setValue',selectedRows[0].remark);
            $("#edit-position").textbox('setValue',selectedRows[0].position);
            $("#edit-email").textbox('setValue',selectedRows[0].email);
            $("#editView").dialog({title:"修改维保员"});
	...
	$("#btnFreeze").bind("click",function(){
      	var selectedRow = $("#dataGrid").datagrid("getSelections");
   		if(selectedRow.length <= 0){
			$.messager.alert("提示", "请选择要冻结的维保员", "error");
			...
			$.messager.confirm("提示","确定要冻结吗?",function(r){
	      		if(r){
					$.post("freezeOrActiveMaintenceAccount",{"id":JsonData.id},function(data){
						if(data=="success"){
    ...
    $("#resetPassword").bind("click",function(){
        var selectedRow = $("#dataGrid").datagrid("getSelections");
        if(selectedRow.length <= 0){
            $.messager.alert("提示", "请选择要重置密码的维保员", "error");
        } else{
            var JsonData = selectedRow[0];
            $.messager.confirm("提示","确定要重置密码吗?",function(r){
                if(r){
                    $.post("maintence_reset_password",{"id":JsonData.id},function(data){
                        if(data=="success"){
      	...
      	var selectedRow = $("#dataGrid").datagrid("getSelections");
   		if(selectedRow.length <= 0){
			...
			$.messager.confirm("提示","确定要启用吗?",function(r){
	      		if(r){
					$.post("freezeOrActiveMaintenceAccount",{"id":JsonData.id,operationType:'active'},function(data){
						if(data=="success"){
