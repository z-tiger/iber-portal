$(function() {
			...
			$("#btnQuery").bind("click",function(){
			    $("#dataGrid").datagrid("load",{
					'rescuerName':$("#rescuerName").textbox("getValue"),
			...
			$("#btnExport").bind("click", function() {
				var rescuerName = $("#rescuerName").textbox("getValue");
			    var cityCode = $("#cityCode").combobox("getValue");
			    var status = $("#status").combobox("getValue");
				$("#rescuerForm").form("submit", {
					url : "export_rescuer_report?rescuerName="+rescuerName+"&cityCode="+cityCode+"&status="+status,
					onSubmit : function() {
					},
	...
	$("#btnAdd").bind("click",function(){
		...
		$("#addView").dialog({title:"添加救援员",width: 700,height: 400});
		$("#addView").dialog("open");
	});
	...
	$("#btnEdit").bind("click",function(){
		...
		if(selectedRows.length <= 0){
			...
			$("#edit-rescuerName").textbox('setValue',selectedRows[0].name);
      	...
      	var selectedRow = $("#dataGrid").datagrid("getSelections");
   		if(selectedRow.length <= 0){
			...
			$.messager.confirm("提示","确定要冻结吗?",function(r){
	      		if(r){
					$.post("freezeOrActiveRescuerAccount",{"id":JsonData.id},function(data){
						if(data=="success"){
    ...
    $("#resetPassword").bind("click",function(){
        ...
        if(selectedRow.length <= 0){
            ...
            $.messager.confirm("提示","确定要重置密码吗?",function(r){
                if(r){
                    $.post("rescuer_reset_password",{"id":JsonData.id},function(data){
                        if(data=="success"){
      	...
      	var selectedRow = $("#dataGrid").datagrid("getSelections");
   		if(selectedRow.length <= 0){
			...
			$.messager.confirm("提示","确定要启用吗?",function(r){
	      		if(r){
					$.post("freezeOrActiveRescuerAccount",{"id":JsonData.id,operationType:'active'},function(data){
						if(data=="success"){
